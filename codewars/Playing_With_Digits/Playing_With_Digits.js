...
function digPow(n, p){
    ...
    // Convert integer n into array of individual digits
    let digits = (n).toString().split("").map(function(digit){
        return parseInt(digit)
    })

    // Iterate through the array and apply the powers (p)
    for (let i = 0, j = p; i < digits.length; i++, j++) {
        total+= Math.pow(digits[i],j);
    }
